...
function loadBlockbusterScript(forceFail) {
  ...
  const downloadScript = new Promise((resolve, reject) => {
    setTimeout(() => {
      console.log("Downloading Script. Please hold...");
      if (forceFail) {
        reject("Download Errored. Please try again");
      } else {
        resolve(scriptData);
      }
    }, 3000);
...
function handleScriptLoad(shouldFail = false) {
  ...
  statusDiv.textContent =
    "Cameras rolling... Downloading script from Hollywood...";
...
function updateWithError(error) {
  statusDiv.textContent =
    "Script loading failed! Check the console for details.";
  ...
  loadButton.innerHTML =
    "LIGHTS, CAMERA, ACTION!<br><span>Load The Script</span>";
  failButton.textContent = "SIMULATE FAILURE";
}
